    ...
    message[75]= 16'h13_e5; //COM8, enable AGC / AEC
	 ...
	 write_address: if(ack==2'b11) begin 
	  ...
	  digest_loop: if(ack==2'b11) begin //stop sccb transmission
		...
		vsync_fedge: if(vsync_1==0 && vsync_2==1) state_d=byte1; //borda de descida de VSYNC: inicio de quadro
				byte1: begin
							if(vsync_1==1 && vsync_2==1) begin
								state_d=vsync_fedge;
							end
							else if(pclk_1==1 && pclk_2==0 && href_1==1 && href_2==1) begin
				...
				byte2: begin
							...
							else if(!(href_1==1 && href_2==1)) begin
								//Linha terminou com pixel incompleto: descarta o primeiro byte
								state_d=byte1;
							end
							else if(pclk_1==1 && pclk_2==0) begin
								pixel_d[7:0]=cmos_db;
								state_d=fifo_write;
							end
						 end
	 ...
	 assign cmos_pwdn=0; 
	 assign cmos_rst_n=1;
	 //O clock externo da OV7670 e gerado no modulo principal por pll_test
	 //e enviado diretamente ao pino MIPI_REFCLK. Esta saida nao e utilizada.
	 assign cmos_xclk=1'b0;
	 assign led=led_q;
